...
(function () {
  ...
  async function login(n, e, p) {
    ...
    if (user && !user.isAnonymous && normEmail(user.email) === e) {
      try { await user.updatePassword(p); await activate(user, n, e); return; }
      catch (error) { if (error?.code !== "auth/requires-recent-login") throw error; await sendLink(e); msg("Link verifikasi sudah dikirim. Buka link tersebut di perangkat ini, lalu masukkan kata sandi yang sama lagi."); return; }
    }
